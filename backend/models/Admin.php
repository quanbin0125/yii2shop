...
//    }
    ...
    public function getMenuItems(){
        //当前登录用户模型
        $menuItems=[];//定义一个空数组装所有的菜单
        //找到所有的一级分类菜单
        $menus=Menu::find()->where(['parent_id'=>0])->all();
        //循环遍历
        foreach($menus as $menu){
            //一级菜单和二级菜单是一对多的关系,但是是在同一个模型当中
            $items=[];
            foreach($menu->children as $son) {
                //判断当前用户是否拥有该权限
